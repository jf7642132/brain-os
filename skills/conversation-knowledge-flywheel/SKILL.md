...
- Prefer upstream read order for 03:00: shared nightly digest → `99-system/03-integration-reports/YYYY-MM-DD/` → stable indexes only if needed
...
- 1 machine-facing run report → `99-system/03-integration-reports/YYYY-MM-DD/conversation-mining-report-YYYY-MM-DD.md`
...
- `99-system/03-integration-reports/YYYY-MM-DD/conversation-mining-report-YYYY-MM-DD.md`
...
**Decision: Direct write vs. Writer-Agent**

| Condition | Action |
|-----------|--------|
| Knowledge items are operational/system notes (bug fixes, config changes, tool quirks) | ✅ Direct write acceptable — these are ephemeral operational records |
| Knowledge items are strategic/domain insights (project decisions, research findings, methodology) | Use Writer-Agent for formal landing |
| Knowledge items will be referenced by other skills or cron jobs | Use Writer-Agent for consistency |
| Uncertain | Default to Writer-Agent |

**Direct write path** (when acceptable):
- Write to `99-system/03-integration-reports/YYYY-MM-DD/conversation-mining-report-YYYY-MM-DD.md`
- Write to `04-queries/daily/04-summary/nightly-digest-YYYY-MM-DD.md`
- Git commit immediately after write
- Verify with `git status --short`

**Writer-Agent path** (preferred for strategic content):
...
- `04-queries/daily/01-learning-suggestions/daily-learning-suggestions.md` — daily suggestions index
...
### Session Discovery in Cron Context
...
See `references/session-discovery-workflow.md` for the validated 5-step workflow:
1. Discover recent sessions (direct file scan, exclude cron)
2. Prioritize by size (largest first)
3. Filter by time window (epoch-based comparison)
4. Extract knowledge from assistant messages (pattern matching)
5. Cross-reference with state.db for metadata

## Implementation Notes: Session Storage Structure
...
- Example: `session_20260429_024309_8139c3.json`
- The embedded timestamp in the filename is the session start time
- Some timestamp-based sessions may NOT be registered in state.db

**UUID-based naming** (Hermes Web UI / API Server sessions):
- Format: `session_<uuid>.json`
- Example: `session_2e490ca6-3f8b-4d9d-a511-382eca7673b2.json`
- No date information in filename; rely on file mtime or state.db
- These are typically sources `api_server`, and ARE registered in state.db
- Often represent the same user conversation split across multiple files via context compaction

### Cron Task Filtering

Cron task sessions can be reliably identified and should be excluded when looking for meaningful human conversations:
- **Pattern 1**: The `session_cron_` prefix in the filename
- **Pattern 2**: The specific pattern `session_cron_ce8a1963857a` for Chronicle Agent's own cron tasks
- **Action**: Skip these when scanning for substantive user conversations

### Knowledge Extraction Patterns

To identify knowledge items in assistant messages, look for these patterns:
- **Skill updates**: "Updated the", "Successfully updated", "I have updated"
- **Learnings**: "Key learning", "What was learned", "Key learnings"

Example extraction logic:
```python
if 'Updated the' in content or 'Successfully updated' in content:
    # Skill update knowledge item
elif 'Key learning' in content or 'What was learned' in content:
    # Learning knowledge item
```
...
-- Filter for non-cron sessions in time window
SELECT * FROM sessions 
WHERE source != 'cron' 
AND started_at >= '2026-04-28T03:00:00'
AND ended_at <= '2026-04-29T03:00:00';
```

### Time-based File Filtering Caveat

The `-mmin -1440` approach for finding files modified in the last 24 hours can be unreliable because files can have historical modification times (e.g., files created at 2026-04-27 01:02 but system time is 2026-04-29).

**More robust approaches:**

1. **Epoch-based comparison with `find`**:
   ```bash
   find /path -printf '%T@ %p\n' | awk -v now=$(date +%s) -v threshold=86400 '$1 > (now - threshold) {print $2}'
   ```

2. **Direct epoch comparison with `stat`**:
   ```bash
...
3. **Human-readable verification**:
   ```bash
   stat -c '%T+ %n' /path/to/file  # Shows modification time in human-readable format
   ```

This is particularly important when:
- Files are batch-created or have historical modification times
- The system clock may differ from file metadata
- You need precise 24-hour window detection

### ⚠️ Pitfall: session_search Time Window Limitations

**Problem**: `session_search` with exact time windows (e.g., "last 24 hours") may return 0 results even when relevant sessions exist, because:
- Session files may be named with different timestamp formats
- Sessions may be stored in non-standard locations
- The search index may not cover the exact window

**Worse: session_search is blind to user sessions in cron context**
In cron job context, session_search almost exclusively returns cron sessions. User-initiated sessions (Telegram, WeChat, API server, CLI) with UUID-based or timestamp-based filenames are completely invisible to session_search. **Do not use session_search as the primary recall mechanism for this task.**

**Solution**: The reliable approach is a direct file scan of the session directory, not session_search:
...
3. **Cross-reference with state.db** for session metadata (source, message count):
   ```sql
   SELECT id, source, started_at, message_count, title FROM sessions
   WHERE source NOT IN ('cron') ORDER BY started_at DESC LIMIT 30;
   ```

4. **Read UUID-based sessions directly** by parsing JSON keys: `['session_id', 'model', 'base_url', 'platform', 'session_start', 'messages']`
5. **Read timestamp-based sessions** (not in state.db) by loading the JSON file from the sessions directory directly
