...
# Backup script for /root/.hermes with exclusions
...
# Create exclude file
EXCLUDE_FILE="/tmp/hermes-backup-exclude.txt"
cat > "$EXCLUDE_FILE" << 'EOF'
# Exclude large state snapshots (can be regenerated)
state-snapshots/
# Exclude node backup
node/bin/node.backup
# Exclude git objects (can be regenerated)
.git/objects/pack/
# Exclude node_modules (can be regenerated)
node/node_modules/
EOF
...
tar -czf "$BACKUP_FILE" --exclude-from="$EXCLUDE_FILE" -C /root .hermes 2>>"$LOG_FILE"
